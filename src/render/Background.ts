...
export class Background {
   ...
   */
  setThemeColors(colors: { primary: string; secondary: string }, themeId?: string): void {
    ...
    if (themeId) {
      ...
      } else if (themeId === 'void_background' && this.voidPortals.length === 0) {
  ...
  private renderStarLayer(): void {
    ...
    for (const star of this.stars) {
      ...
      const brightness = Math.max(minBrightness, star.brightness + twinkle * 0.3);
      ...
      if (this.currentThemeId === 'nebula_background') {
        ...
        if (colorIndex === 0) starColor = [255, 150, 150]; // Red
        else if (colorIndex === 1) starColor = [150, 200, 255]; // Blue
        else if (colorIndex === 2) starColor = [255, 255, 150]; // Yellow
  ...
  private renderNebulaBase(ctx: CanvasRenderingContext2D): void {
    ...
    for (const cloud of this.nebulaClouds) {
      ...
      gradient.addColorStop(0, `hsla(${Math.round(cloud.color1)}, 70%, ${Math.round(cloud.lightness1)}%, 0.5)`);
      gradient.addColorStop(0.5, `hsla(${Math.round(cloud.color2)}, 60%, ${Math.round(cloud.lightness2)}%, 0.25)`);
      gradient.addColorStop(1, `hsla(${Math.round(cloud.color2)}, 60%, ${Math.round(cloud.lightness2)}%, 0)`);
  ...
  private renderNebulaOverlay(ctx: CanvasRenderingContext2D): void {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const wispGradient = ctx.createRadialGradient(wispX, wispY, 0, wispX, wispY, 150);
      const hue = 260 + i * 15;
      // Use hsla() format with proper opacity values (0-1 range)
      wispGradient.addColorStop(0, `hsla(${hue}, 70%, 50%, ${(0.3 * wispPulse).toFixed(3)})`);
  ...
  private renderVoidOverlay(ctx: CanvasRenderingContext2D): void {
    ...
    for (const orb of this.energyOrbs) {
      ...
      // Outer glow
      const glowGradient = ctx.createRadialGradient(orb.x, orb.y, 0, orb.x, orb.y, currentRadius * 3);
