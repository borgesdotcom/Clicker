...
export type TutorialStep = 'click_alien' | 'buy_upgrade' | 'boss_warning' | 'completed';
...
export class TutorialSystem {
    ...
    private showStep(step: TutorialStep): void {
        ...
        switch (step) {
            case 'click_alien':
                this.highlightElement('game-canvas', 'Click the Alien to earn points!', 'top');
                break;
            case 'buy_upgrade':
                ...
                if (shopPanel && !isCollapsed) {
                    ...
                    this.highlightElement('shop-content', 'Buy your first upgrade!', 'left');
                } else {
                    // Shop is closed, point to the toggle
                    this.highlightElement('desktop-shop-toggle', 'Open the Shop to buy upgrades!', 'right');
                }
                break;
            case 'boss_warning':
                // This is triggered by external events (boss spawn)
                this.highlightElement('boss-timer-hud', 'Defeat the Boss before time runs out!', 'bottom');
                break;
            case 'completed':
                this.complete();
                break;
    ...
    private highlightElement(elementId: string, text: string, position: 'top' | 'bottom' | 'left' | 'right'): void {
